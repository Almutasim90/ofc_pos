...
public static class SprintTwoEndpoints
{
    ...
    private static async Task<IResult> CreateProduct(ProductRequest request, OFCDbContext db, IdentityService identity, ClaimsPrincipal user, HttpContext context, CancellationToken ct)
    {
        ...
        var product = new Product { Sku = request.Sku.Trim(), Barcode = string.IsNullOrWhiteSpace(request.Barcode) ? "OFC" + Guid.CreateVersion7().ToString("N") : request.Barcode.Trim(), NameAr = request.NameAr.Trim(), NameEn = request.NameEn.Trim(), DescriptionAr = request.DescriptionAr?.Trim(), DescriptionEn = request.DescriptionEn?.Trim(), CategoryId = request.CategoryId, Type = request.Type, TaxCategoryId = request.TaxCategoryId, PreparationStationId = request.PreparationStationId, BasePrice = request.BasePrice, IsActive = request.IsActive ?? true };
    ...
    private static async Task<IResult> UpdateProduct(Guid id, ProductRequest request, OFCDbContext db, IdentityService identity, ClaimsPrincipal user, HttpContext context, CancellationToken ct)
    {
        ...
        product.Sku = request.Sku.Trim(); product.Barcode = string.IsNullOrWhiteSpace(request.Barcode) ? product.Barcode ?? "OFC" + product.Id.ToString("N") : request.Barcode.Trim(); product.NameAr = request.NameAr.Trim(); product.NameEn = request.NameEn.Trim(); product.DescriptionAr = request.DescriptionAr?.Trim(); product.DescriptionEn = request.DescriptionEn?.Trim(); product.CategoryId = request.CategoryId; product.Type = request.Type; product.TaxCategoryId = request.TaxCategoryId; product.PreparationStationId = request.PreparationStationId; product.BasePrice = request.BasePrice; if (request.IsActive is not null) product.IsActive = request.IsActive.Value;
        product.Images.Clear(); foreach (var image in request.Images ?? []) db.ProductImages.Add(new ProductImage { ProductId = product.Id, Url = image.Url.Trim(), SortOrder = image.SortOrder, IsPrimary = image.SortOrder == 0 });
