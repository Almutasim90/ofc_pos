...
public static class SprintOneEndpoints
{
    ...
    {
        var error = Validate(request.OrganizationNameAr, "organizationNameAr", 160) ?? Validate(request.OrganizationNameEn, "organizationNameEn", 160) ?? Validate(request.BranchNameAr, "branchNameAr", 160) ?? Validate(request.BranchNameEn, "branchNameEn", 160) ?? ValidateEmail(request.Email) ?? ValidatePassword(request.Password);
        if (error is not null) return error;
        try { var user = await identity.BootstrapAsync(request.OrganizationNameAr, request.OrganizationNameEn, request.BranchNameAr, request.BranchNameEn, request.Email, request.DisplayName, request.Password, Correlation(context), ct); return Results.Created($"/api/v1/users/{user.Id}", new { user.Id, user.Email, user.DisplayName }); }
    ...
    {
        if (string.IsNullOrWhiteSpace(request.Email) || string.IsNullOrWhiteSpace(request.Password)) return Validation("email", "Email and password are required.");
        var result = await identity.LoginAsync(request.Email, request.Password, request.BranchId, request.DeviceId, Correlation(context), ct);
        ...
        return Results.Ok(new { session.Token, user = new { session.User.Id, session.User.Email, session.User.DisplayName }, session.BranchId, session.DeviceId });
    }
    ...
    private static IResult? ValidateEmail(string email) => string.IsNullOrWhiteSpace(email) || !System.Net.Mail.MailAddress.TryCreate(email, out _) ? Validation("email", "A valid email address is required.") : null;
    private static IResult? ValidatePassword(string password) => password.Length < 12 ? Validation("password", "Password must contain at least 12 characters.") : null;
    ...
    private sealed record BootstrapRequest(string OrganizationNameAr, string OrganizationNameEn, string BranchNameAr, string BranchNameEn, string Email, string DisplayName, string Password);
    private sealed record LoginRequest(string Email, string Password, Guid? BranchId, Guid? DeviceId);
    ...
    private sealed record CreateUserRequest(string Email, string DisplayName, string Password, List<Guid> RoleIds, List<Guid> BranchIds);
