...
export function PosSection({ language, kiosk, onKioskChange }: { language: Language; kiosk: boolean; onKioskChange: (value: boolean) => void }) {
  ...
  const heldOrdersModal = heldOpen && <div className="fixed inset-0 z-50 grid place-items-end bg-black/35 sm:place-items-center sm:p-5"><section role="dialog" aria-modal="true" aria-label={t.heldOrders} className="max-h-[85vh] w-full max-w-md overflow-y-auto rounded-t-2xl bg-white p-5 shadow-2xl sm:rounded-2xl">{detailOrderId ? orderDetailView : <><div className="flex items-center justify-between"><h2 className="text-lg font-bold">{t.heldOrders}</h2><button onClick={closeHeldOrders} className="grid size-9 place-items-center rounded-lg bg-[#f4f7f4]">×</button></div><label className="mt-3 flex min-h-11 items-center gap-2 rounded-xl border border-[#cdd7d0] bg-white px-3"><Search size={16} /><input aria-label={t.heldSearch} value={heldSearch} onChange={(e) => setHeldSearch(e.target.value)} placeholder={t.heldSearch} className="w-full bg-transparent text-sm outline-none" /></label>{heldOrders.length === 0 ? <p className="mt-6 text-center text-sm text-[#69766f]">{t.noHeld}</p> : matchingHeldOrders.length === 0 ? <p className="mt-6 text-center text-sm text-[#69766f]">{t.heldNoMatch}</p> : <ul className="mt-4 space-y-2">{matchingHeldOrders.map((order) => <li key={order.id} className="rounded-xl bg-[#f4f7f4] p-3"><button type="button" onClick={() => void openOrderDetail(order.id)} className="flex w-full items-center justify-between gap-3 text-start"><div><p className="font-semibold">{orderRef(order.id)} · OMR {order.grossAmount.toFixed(3)}{order.table && <span className="ms-2 rounded-full bg-white px-2 py-0.5 text-xs font-semibold text-[#0e5a4f]">{t.table} {order.table.code}</span>}</p><p className="mt-0.5 text-xs text-[#69766f]">{statusLabels[order.status]} · {new Date(order.createdAt).toLocaleTimeString(language)}{order.note ? ` · ${order.note}` : ""}</p></div></button><div className="mt-2 flex flex-wrap gap-2"><button disabled={busy} onClick={() => void resumeHeld(order, true)} className="min-h-11 rounded-lg border px-3 text-sm">{language === "ar" ? "إرسال للمطبخ" : "Send to kitchen"}</button>{order.status !== "Paid" && <button disabled={busy} onClick={() => void resumeHeld(order)} className="min-h-10 shrink-0 rounded-lg bg-[#0e5a4f] px-3 text-sm font-semibold text-white">{t.send}</button>}</div></li>)}</ul>}</>}</section></div>;
