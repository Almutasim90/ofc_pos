...
export function PosSection({ language, kiosk, onKioskChange }: { language: Language; kiosk: boolean; onKioskChange: (value: boolean) => void }) {
  ...
  const heldOrdersModal = heldOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-black/35 p-3 sm:p-5"><section role="dialog" aria-modal="true" aria-label={t.heldOrders} className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">{detailOrderId ? <div className="overflow-y-auto p-5">{orderDetailView}</div> : <>
