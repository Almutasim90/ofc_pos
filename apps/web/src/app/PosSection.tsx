...
const words = { ar: { title: "نقطة البيع", search: "ابحث عن صنف", all: "الكل", cart: "السلة", empty: "أضف أصنافًا للبدء", hold: "تعليق", send: "الدفع", notes: "ملاحظة", branch: "الفرع", channel: "قناة البيع", offline: "غير متصل: ستتم المزامنة عند عودة الاتصال", online: "متصل", total: "الإجمالي", add: "إضافة", confirm: "تأكيد الاختيارات", selections: "الاختيارات", saved: "تم حفظ الطلب", unavailable: "تعذر حفظ الطلب، سيبقى في السلة", sessionExpired: "انتهت صلاحية الجلسة. سجّل الدخول من جديد؛ الطلب سيبقى في السلة.", viewCart: "عرض السلة", offlinePayTitle: "دفع غير متصل", offlinePayMethod: "طريقة الدفع", offlinePayTendered: "المبلغ المستلم", offlinePayConfirm: "تأكيد الدفع وحفظ الطلب", offlinePayCancel: "إلغاء", offlineNoMethods: "لا توجد وسائل دفع محفوظة لهذا الفرع؛ اتصل بالإنترنت مرة واحدة على الأقل", offlinePayInvalid: "المبلغ المستلم غير كافٍ", heldOrders: "الطلبات الحالية", resume: "استئناف", noHeld: "لا توجد طلبات حالية", heldSince: "منذ", orderRef: "رقم الطلب", back: "رجوع", loadingOrder: "جارٍ تحميل تفاصيل الطلب…", orderItems: "الأصناف", orderNet: "صافي المبلغ", orderTax: "الضريبة", dailySales: "مبيعات اليوم", dailySalesOrders: "طلب", qrNew: "وصل طلب QR جديد", qrPending: "طلب QR جديد بانتظار الاعتماد", qrApproved: "تم اعتماد طلب QR", qrRejected: "تم رفض طلب QR", qrReviewTitle: "طلبات QR بانتظار الاعتماد", qrApprove: "اعتماد", qrReject: "رفض", tableNumber: "رقم الطاولة", tableNumberPlaceholder: "مثال: 5", tableSearch: "ابحث برقم الطاولة", tableSearchEmpty: "لا توجد طلبات بهذا الرقم", paid: "مدفوع", unpaid: "غير مدفوع" }, en: { title: "Point of sale", search: "Search products", all: "All", cart: "Cart", empty: "Add products to begin", hold: "Hold", send: "Pay", notes: "Note", branch: "Branch", channel: "Sales channel", offline: "Offline: the cart will sync when connection returns", online: "Online", total: "Total", add: "Add", confirm: "Confirm selections", selections: "Selections", saved: "Order saved", unavailable: "Unable to save; cart remains available", sessionExpired: "Your session has expired. Please sign in again; your cart will stay saved.", viewCart: "View cart", offlinePayTitle: "Offline payment", offlinePayMethod: "Payment method", offlinePayTendered: "Amount tendered", offlinePayConfirm: "Confirm payment and save order", offlinePayCancel: "Cancel", offlineNoMethods: "No payment methods are cached for this branch; connect to the internet at least once first", offlinePayInvalid: "Tendered amount does not cover the total", heldOrders: "Current orders", resume: "Resume", noHeld: "No current orders", heldSince: "Held since", orderRef: "Order ref", back: "Back", loadingOrder: "Loading order details…", orderItems: "Items", orderNet: "Net amount", orderTax: "Tax", dailySales: "Today's sales", dailySalesOrders: "orders", qrNew: "New QR order received", qrPending: "New QR order awaiting approval", qrApproved: "QR order approved", qrRejected: "QR order rejected", qrReviewTitle: "QR orders awaiting approval", qrApprove: "Approve", qrReject: "Reject", tableNumber: "Table number", tableNumberPlaceholder: "e.g. 5", tableSearch: "Search by table number", tableSearchEmpty: "No orders for this table", paid: "Paid", unpaid: "Unpaid" } } as const;
type HeldOrder = { id: string; status: string; grossAmount: number; note: string | null; tableNumber: string | null; createdAt: string };
...
export function PosSection({ language, kiosk, onKioskChange }: { language: Language; kiosk: boolean; onKioskChange: (value: boolean) => void }) {
        ...
        {pendingQr.map((o) => (
          <li key={o.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-white p-2.5 text-sm shadow-sm">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 truncate text-xs text-[#69766f]">{o.lines.map((l) => `${l.quantity}× ${language === "ar" ? l.productNameAr : l.productNameEn}`).join("، ")}</p>
            </div>
  ...
  async function submit(status: "Draft" | "Pending" | "Kitchen") {
    ...
    if (!online) {
      ...
      if (status === "Draft") { enqueue({ operationType: "order.create", baseVersion: null, baseCatalogVersion: null, occurredAt: new Date().toISOString(), payload: buildOfflineOrder(channelId, cart, "Draft") }); setCart([]); setTableNumber(""); setMessage(t.offline); return; }
    ...
    try {
      ...
      const body = { branchId, salesChannelId: channelId, clientRequestId: requestRef.current.id, source: "Pos", note: null, tableNumber: tableNumber.trim() || null, lines: cart.map(line => ({ productId: line.product.id, quantity: line.quantity, note: line.note || null, selections: Object.entries(line.selections).map(([selectionGroupId, ids]) => ({ selectionGroupId, choices: ids.map(optionId => ({ optionId, quantity: 1 })) })) })) };
      ...
      setCart([]); setTableNumber(""); requestRef.current = { snapshot: "", id: "" };
      if (status !== "Draft") {
  ...
  function confirmOfflinePayment() {
    ...
    setCart([]); setTableNumber(""); setOfflinePay(null); setMessage(t.offline);
  }
  ...
  function closeHeldOrders() { setHeldOpen(false); setDetailOrderId(null); setOrderDetail(null); }
  const orderDetailView = detailOrderId && <><div className="flex items-center gap-2"><button onClick={() => { setDetailOrderId(null); setOrderDetail(null); }} className="grid size-9 place-items-center rounded-lg bg-[#f4f7f4]">{language === "ar" ? "→" : "←"}</button><h2 className="text-lg font-bold">{t.orderRef} {orderRef(detailOrderId)}</h2></div>{!orderDetail ? <p role="status" className="mt-6 text-center text-sm text-[#69766f]">{t.loadingOrder}</p> : <div className="mt-4 space-y-4"><p className="text-sm text-[#69766f]">{statusLabels[orderDetail.status] ?? orderDetail.status} · {new Date(orderDetail.createdAt).toLocaleString(language)}{orderDetail.note ? ` · ${orderDetail.note}` : ""}</p><div><p className="text-sm font-semibold">{t.orderItems}</p><ul className="mt-2 space-y-2">{orderDetail.lines.map((line) => <li key={line.id} className="rounded-xl bg-[#f4f7f4] p-3"><div className="flex justify-between gap-3"><span>{line.quantity} × {language === "ar" ? line.productNameAr : line.productNameEn}</span><span>OMR {(line.unitGrossAmount * line.quantity).toFixed(3)}</span></div>{line.note && <p className="mt-1 text-xs text-[#69766f]">{line.note}</p>}</li>)}</ul></div><div className="space-y-1 border-t border-[#dfe5df] pt-3 text-sm"><div className="flex justify-between text-[#69766f]"><span>{t.orderNet}</span><span>OMR {orderDetail.netAmount.toFixed(3)}</span></div><div className="flex justify-between text-[#69766f]"><span>{t.orderTax}</span><span>OMR {orderDetail.taxAmount.toFixed(3)}</span></div><div className="flex justify-between text-lg font-bold"><span>{t.total}</span><span>OMR {orderDetail.grossAmount.toFixed(3)}</span></div></div></div>}</>;
  const filteredHeld = tableSearch.trim() ? heldOrders.filter((o) => o.tableNumber?.toLowerCase().includes(tableSearch.trim().toLowerCase())) : heldOrders;
  const heldOrdersModal = heldOpen && <div className="fixed inset-0 z-50 grid place-items-end bg-black/35 sm:place-items-center sm:p-5"><section role="dialog" aria-modal="true" aria-label={t.heldOrders} className="max-h-[85vh] w-full max-w-md overflow-y-auto rounded-t-2xl bg-white p-5 shadow-2xl sm:rounded-2xl">{detailOrderId ? orderDetailView : <><div className="flex items-center justify-between"><h2 className="text-lg font-bold">{t.heldOrders}</h2><button onClick={closeHeldOrders} className="grid size-9 place-items-center rounded-lg bg-[#f4f7f4]">×</button></div><label className="mt-3 flex min-h-11 items-center gap-2 rounded-xl border border-[#cdd7d0] bg-white px-3"><Search size={16} /><input aria-label={t.tableSearch} value={tableSearch} onChange={(e) => setTableSearch(e.target.value)} placeholder={t.tableSearch} className="w-full bg-transparent text-sm outline-none" /></label>{heldOrders.length === 0 ? <p className="mt-6 text-center text-sm text-[#69766f]">{t.noHeld}</p> : filteredHeld.length === 0 ? <p className="mt-6 text-center text-sm text-[#69766f]">{t.tableSearchEmpty}</p> : <ul className="mt-4 space-y-2">{filteredHeld.map((order) => <li key={order.id} className="rounded-xl bg-[#f4f7f4] p-3"><button type="button" onClick={() => void openOrderDetail(order.id)} className="flex w-full items-center justify-between gap-3 text-start"><div><p className="font-semibold">{orderRef(order.id)} · OMR {order.grossAmount.toFixed(3)}{order.tableNumber && <span className="ms-2 rounded-full bg-[#eef3ee] px-2 py-0.5 text-xs font-semibold text-[#0e5a4f]">{t.tableNumber}: {order.tableNumber}</span>}</p><p className="mt-0.5 text-xs"><span className={order.status === "Paid" ? "font-semibold text-[#137347]" : "font-semibold text-[#b4322a]"}>{order.status === "Paid" ? t.paid : t.unpaid}</span><span className="text-[#69766f]"> · {statusLabels[order.status]} · {new Date(order.createdAt).toLocaleTimeString(language)}{order.note ? ` · ${order.note}` : ""}</span></p></div></button><div className="mt-2 flex flex-wrap gap-2"><button disabled={busy} onClick={() => void resumeHeld(order, true)} className="min-h-11 rounded-lg border px-3 text-sm">{language === "ar" ? "إرسال للمطبخ" : "Send to kitchen"}</button>{order.status !== "Paid" && <button disabled={busy} onClick={() => void resumeHeld(order)} className="min-h-10 shrink-0 rounded-lg bg-[#0e5a4f] px-3 text-sm font-semibold text-white">{t.send}</button>}</div></li>)}</ul>}</>}</section></div>;
