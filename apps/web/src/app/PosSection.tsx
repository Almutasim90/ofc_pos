...
type Product = { id: string; categoryId: string; categoryNameAr: string; categoryNameEn: string; nameAr: string; nameEn: string; basePrice: number | null; imageUrl: string | null; pricing: Pricing; selectionGroups: Group[] };
...
export function PosSection({ language }: { language: Language }) {
  ...
  useEffect(() => { if (!branchId) return; void (async () => { const response = await auth(`/api/v1/payment-methods?branchId=${branchId}`); if (!response.ok) return; const value = await response.json() as Method[]; setOfflineMethods(value); store.set("pos-payment-methods", value); })(); }, [branchId]);
  useEffect(() => { store.set("pos-cart", cart); }, [cart]);
  async function loadHeld() { if (!branchId || !online) return; const response = await auth(`/api/v1/orders?branchId=${branchId}`); if (response.ok) setHeldOrders((await response.json() as HeldOrder[]).filter((o) => o.status === "Draft")); }
  useEffect(() => { void loadHeld(); }, [branchId, online]);
  async function resumeHeld(order: HeldOrder) {
    setMessage("");
    const response = await auth(`/api/v1/orders/${order.id}/status`, { method: "POST", body: JSON.stringify({ status: "Pending", note: null }) });
    if (!response.ok) { setMessage(t.unavailable); return; }
    setHeldOpen(false); setPayment({ orderId: order.id, total: order.grossAmount }); void loadHeld();
  }
  ...
  async function submit(status: "Draft" | "Pending") {
    ...
    if (!online) {
      ...
      // A held (Draft) order never needs a payment; a "Pay" tap while offline still has to name a
      // real payment method and tendered amount before we mark the order Paid (see confirmOfflinePayment).
      setOfflinePay({ methodId: offlineMethods[0]?.id ?? "", tendered: total.toFixed(3) });
      return;
    }
    const body = { branchId, salesChannelId: channelId, clientRequestId: createId(), source: "Pos", note: null, lines: cart.map((line) => ({ productId: line.product.id, quantity: line.quantity, note: line.note || null, selections: Object.entries(line.selections).map(([selectionGroupId, ids]) => ({ selectionGroupId, choices: ids.map((optionId) => ({ optionId, quantity: 1 })) })) })) };
    const response = await auth("/api/v1/orders", { method: "POST", body: JSON.stringify(body) }); if (!response.ok) { setMessage(t.unavailable); return; } const order = await response.json() as { id: string }; if (status === "Pending") { const statusResponse = await auth(`/api/v1/orders/${order.id}/status`, { method: "POST", body: JSON.stringify({ status: "Pending", note: null }) }); if (!statusResponse.ok) { setMessage(t.unavailable); return; } setPayment({ orderId: order.id, total }); } else { void loadHeld(); } setCart([]); setMessage(t.saved);
  }
