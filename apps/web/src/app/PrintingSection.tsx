...
type Config = { id: string; code: string; nameAr: string; nameEn: string; kind: PrinterKind; deviceName: string | null; isActive: boolean; sortOrder: number };
        ...
        <div className="mt-6 grid gap-5 xl:grid-cols-2">
          ...
          <section className="rounded-xl border border-[#dfe5df] bg-white p-5">
            ...
            {configs.length === 0 ? <p className="mt-3 text-sm text-[#69766f]">{t.noConfigs}</p> : (
              <ul className="mt-3 divide-y divide-[#e8ece8]">{configs.map((c) => <li key={c.id} className="flex flex-wrap items-center justify-between gap-2 py-3"><div className="min-w-0"><p className="font-medium">{name(c)}</p><p className="text-sm text-[#69766f]">{c.code} · {printerKindLabel(c.kind)}{c.deviceName ? ` · ${c.deviceName}` : ""}</p></div><span className={`rounded-full px-3 py-1 text-xs font-semibold ${c.isActive ? "bg-[#e3f4ea] text-[#137347]" : "bg-[#e8ece8] text-[#53615b]"}`}>{c.isActive ? t.active : t.inactive}</span></li>)}</ul>
            )}
          </section>
        </div>
      )}
