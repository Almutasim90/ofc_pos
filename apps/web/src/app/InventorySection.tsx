...
export function InventorySection({ language, permissions }: { language: Language; permissions: string[] | null }) {
  const t = copy[language];
  const can = (code: string) => permissions !== null && permissions.includes(code);
  const canUoms = can("inventory.uoms.manage");
  const canItems = can("inventory.items.manage");
  const canRecipes = can("inventory.recipes.manage");
  const canMovements = can("inventory.movements.manage");
