...
const copy = {
  ar: {
    title: "المخزون والوصفات", intro: "المواد الخام ووحدات القياس والتحويلات ووصفات المنتجات وسجل الحركات المخزنية.", isolated: "التتبع يتم بسجل حركات غير قابل للتعديل.", loading: "جارٍ التحميل", reload: "تحديث", saved: "تم الحفظ بنجاح.", failed: "تعذر تنفيذ العملية.", empty: "لا توجد بيانات بعد.", none: "لا يوجد",
...
export function InventorySection({ language }: { language: Language }) {
  ...
  async function createUnit(event: React.FormEvent) {
    ...
    try {
      ...
      setMsg(t.saved); setUnitForm({ code: "", nameAr: "", nameEn: "", symbol: "", sortOrder: "" }); await loadUoms();
  ...
  async function createConversion(event: React.FormEvent) {
    ...
    try {
      ...
      setMsg(t.saved); setConvForm({ fromUnitId: "", toUnitId: "", factor: "" }); await loadUoms();
    } catch (e) { setMsg(e instanceof Error ? e.message : t.failed, true); }
  }
  ...
  async function createRecipe(event: React.FormEvent) {
    ...
    try {
      ...
      setMsg(t.saved); setRecipeForm({ productId: "", nameAr: "", nameEn: "", effectiveFrom: "" }); setRecipeLines([]); setDetail(null); await loadRecipes();
    } catch (e) { setMsg(e instanceof Error ? e.message : t.failed, true); }
  }
  ...
  async function recordMovement(event: React.FormEvent) {
    ...
    try {
      ...
      setMsg(t.saved); setMovementForm({ itemId: "", type: "Opening", unitId: "", quantity: "", reference: "", reason: "" }); await Promise.all([loadStock(), loadMovements()]);
    } catch (e) { setMsg(e instanceof Error ? e.message : t.failed, true); }
  }
    ...
    <div>
      ...
      <div className="mt-6 grid gap-5">
        <section hidden={inventoryTab !== "units"} className="rounded-xl border border-[#dfe5df] bg-white p-5"><h2 className="font-semibold">{t.units}</h2>
          ...
          <form onSubmit={createUnit} className="mt-5 rounded-lg border border-[#e8ece8] bg-[#fafbfa] p-4">
            <h3 className="font-semibold">{t.addUnit}</h3>
          ...
          <section className="mt-5 rounded-lg border border-[#e8ece8] bg-[#fafbfa] p-4">
            <h3 className="font-semibold">{t.conversions}</h3>
            ...
            <form onSubmit={createConversion} className="mt-3 grid gap-3 sm:grid-cols-3">
        ...
        <section hidden={inventoryTab !== "recipes"} className="rounded-xl border border-[#dfe5df] bg-white p-5"><h2 className="font-semibold">{t.recipes}</h2>
          ...
          <form onSubmit={createRecipe} className="mt-5 rounded-lg border border-[#e8ece8] bg-[#fafbfa] p-4">
            <h3 className="font-semibold">{t.addRecipe}</h3>
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
        ...
        <section hidden={inventoryTab !== "stock"} className="rounded-xl border border-[#dfe5df] bg-white p-5"><h2 className="font-semibold">{t.stock}</h2>
          ...
          <form onSubmit={recordMovement} className="mt-5 rounded-lg border border-[#e8ece8] bg-[#fafbfa] p-4">
