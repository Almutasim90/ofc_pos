...
const input = "mt-1 min-h-11 min-w-0 w-full rounded-lg border border-[#cdd7d0] bg-white px-3 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0e5a4f] focus-visible:ring-offset-1";
const button = "inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border border-[#cdd7d0] px-4 text-sm font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0e5a4f] focus-visible:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50";
...
export function CatalogScreen({ language, mode }: { language: "ar" | "en"; mode: "products" | "categories" }) {
  ...
  useEffect(() => { setEditing(null); setSearch(""); setMessage(""); setPage(1); void load(); }, [mode]);
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">{isProduct ? tr("قائمة المنتجات", "Products") : tr("تصنيفات القائمة", "Menu categories")}</h1><p className="mt-2 text-sm text-[#000000]">{tr("ابحث عن السجل واضغط تعديل. الحقول ذات النجمة مطلوبة.", "Find a record and choose Edit. Fields marked * are required.")}</p></div><button className={`${button} bg-[#0e5a4f] text-white`} onClick={() => open()}><Plus size={18} />{tr("إضافة جديد", "Add new")}</button></div>
    {message && <p role="status" className="rounded-xl border bg-white p-4 text-sm">{message}</p>}
    <label htmlFor="catalog-search" className="flex items-center gap-2 rounded-xl border border-[#cdd7d0] bg-white px-3 focus-within:ring-2 focus-within:ring-[#0e5a4f] focus-within:ring-offset-1"><Search size={18} aria-hidden="true" /><input id="catalog-search" aria-describedby="catalog-search-hint" placeholder={tr("ابحث بالاسم أو الرمز", "Search by name or code")} value={search} onChange={e => setSearch(e.target.value)} className="min-h-12 min-w-0 flex-1 bg-transparent outline-none" /></label>
    <p id="catalog-search-hint" className="sr-only">{tr("اكتب للتصفية حسب الاسم أو الرمز", "Type to filter by name or code")}</p>
    {loading ? <ul aria-busy="true" role="status" aria-label={tr("جارٍ التحميل…", "Loading…")} className="grid gap-3 xl:grid-cols-2">{Array.from({ length: 6 }).map((_, i) => <li key={i} className="flex h-24 animate-pulse items-center gap-3 rounded-xl border bg-white p-4"><div className="h-16 w-16 shrink-0 rounded-xl bg-[#edf5f1]" /><div className="min-w-0 flex-1 space-y-2"><div className="h-4 w-2/3 rounded bg-[#edf5f1]" /><div className="h-3 w-1/3 rounded bg-[#edf5f1]" /></div></li>)}</ul> : rows.length === 0 ? <p className="rounded-xl border bg-white p-8 text-center">{tr("لا توجد نتائج. أضف سجلًا أو غيّر البحث.", "No results. Add a record or change your search.")}</p> : <>
      <ul className="grid gap-3 xl:grid-cols-2">{pageRows.map(item => <li key={item.id} className="flex min-w-0 flex-wrap items-center gap-3 rounded-xl border bg-white p-4 transition-colors hover:border-[#0e5a4f]/40"><ProductPhoto src={"images" in item ? item.images[0]?.url : item.imageUrl} name={name(item)} /><div className="min-w-0 flex-1"><h2 className="font-semibold break-words">{name(item)}</h2><p className="mt-1 text-sm text-[#000000]">{"sku" in item ? `${item.sku} · ${item.basePrice?.toFixed(3) ?? "—"} OMR` : tr("تصنيف القائمة", "Menu category")}</p><p className="text-xs">{item.isActive ? tr("نشط", "Active") : tr("متوقف", "Inactive")}</p></div><div className="flex flex-wrap gap-2"><button className={button} onClick={() => open(item)}><Pencil size={16} />{tr("تعديل", "Edit")}</button>{"sku" in item && <button className={button} onClick={() => void openGroups(item)}>{tr("الإضافات", "Modifiers")}</button>}</div></li>)}</ul>
      <Pagination page={page} pageSize={PAGE_SIZE} total={rows.length} onPageChange={setPage} language={language} />
    </>}
    ...
    {editing && <FormDialog title={editing === "new" ? tr("إضافة جديد", "Add new") : tr("تعديل البيانات", "Edit details")} closeLabel={tr("إلغاء", "Cancel")} onClose={() => setEditing(null)}>
      <form onSubmit={save} className="space-y-5">
        <div className="grid gap-4 sm:grid-cols-2">{field("nameAr", tr("الاسم بالعربية", "Arabic name"), true)}{field("nameEn", tr("الاسم بالإنجليزية", "English name"), true)}
          {isProduct ? <><SearchableSelect label={tr("التصنيف *", "Category *")} value={form.categoryId} onChange={v => setForm({ ...form, categoryId: v })}><option value="">{tr("اختر التصنيف", "Choose category")}</option>{categories.map(c => <option key={c.id} value={c.id}>{name(c)}</option>)}</SearchableSelect>{field("basePrice", tr("سعر البيع (ر.ع)", "Selling price (OMR)"), editing === "new", "number", tr("السعر قبل تطبيق قواعد التسعير الخاصة بالقناة.", "Before channel-specific pricing rules."))}</> : <SearchableSelect label={tr("التصنيف الرئيسي", "Parent category")} value={form.parentId} onChange={v => setForm({ ...form, parentId: v })}><option value="">{tr("تصنيف مستقل", "Top-level category")}</option>{categories.filter(c => c.id !== editing && !c.parentId).map(c => <option key={c.id} value={c.id}>{name(c)}</option>)}</SearchableSelect>}
        </div>
        {isProduct && <><fieldset><legend className="font-medium">{tr("صور المنتج (اختياري)", "Product photos (optional)")}</legend><p className="mt-1 text-xs text-[#000000]">{tr("ضع رابط الصورة أو اختر صورة من قائمة المطعم. الصورة الأولى تظهر للكاشير والعميل.", "Paste an image link or choose a menu photo. The first image appears to cashiers and customers.")}</p>{images.map((url, i) => <div key={i} className="mt-3 flex items-center gap-3"><ProductPhoto src={url} name={form.nameAr || form.nameEn} /><input aria-label={tr("رابط الصورة", "Image URL")} className={input} value={url} maxLength={500} onChange={e => setImages(images.map((x, j) => j === i ? e.target.value : x))} list="menu-photos" /><button type="button" className={button} onClick={() => setImages(images.length > 1 ? images.filter((_, j) => j !== i) : [""])} aria-label={tr("إزالة الصورة", "Remove image")}>×</button></div>)}<datalist id="menu-photos">{Object.keys(import.meta.glob("/public/menu/**/*.jpg")).map(p => <option key={p} value={p.replace("/public", "")} />)}</datalist><button type="button" className={`${button} mt-3`} onClick={() => setImages([...images, ""])}>{tr("إضافة صورة أخرى", "Add another photo")}</button></fieldset>
        <div><SearchableSelect label={tr("مكان تحضير المنتج", "Where is this product prepared?")} value={form.preparationStationId} onChange={v => setForm({ ...form, preparationStationId: v })}><option value="">{tr("المطبخ العام", "General kitchen")}</option>{stations.map(s => <option key={s.id} value={s.id}>{name(s)}</option>)}</SearchableSelect><span className="mt-1 block text-xs font-normal text-[#000000]">{tr("مثال: البرجر إلى الشواية، والعصير إلى المشروبات. إن لم تحدد مكانًا يظهر المنتج في قائمة المطبخ العامة.", "For example: burgers go to the grill, juice to drinks. Leave blank for the general kitchen queue.")}</span></div></>}
        <details className="rounded-xl border p-4"><summary className="cursor-pointer font-semibold">{tr("تفاصيل إضافية (اختيارية)", "Additional details (optional)")}</summary><div className="mt-4 grid gap-4 sm:grid-cols-2">{isProduct ? <>{field("sku", tr("رمز الصنف", "SKU"), false, "text", tr("يُنشأ تلقائيًا عند تركه فارغًا.", "Generated automatically when blank."))}{field("barcode", tr("الباركود", "Barcode"), false, "text", tr("يُنشأ تلقائيًا. أدخل باركود العبوة إن كان موجودًا.", "Generated automatically. Enter the package barcode if one exists."))}<SearchableSelect label={tr("نوع المنتج", "Product type")} value={form.type} onChange={v => setForm({ ...form, type: v })}><option value="Simple">{tr("منتج عادي", "Standard product")}</option><option value="Combo">{tr("وجبة مركبة", "Combo meal")}</option><option value="Service">{tr("خدمة", "Service")}</option></SearchableSelect>{field("descriptionAr", tr("وصف بالعربية", "Arabic description"))}{field("descriptionEn", tr("وصف بالإنجليزية", "English description"))}</> : <>{field("sortOrder", tr("ترتيب العرض", "Display order"), false, "number")}{field("imageUrl", tr("رابط الصورة", "Image URL"))}</>}</div></details>
