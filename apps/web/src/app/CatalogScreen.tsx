...
import { Plus, Search, Pencil, ArrowLeft, Package } from "lucide-react";
...
export function CatalogScreen({ language, mode }: { language: "ar" | "en"; mode: "products" | "categories" }) {
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-bold">{isProduct ? tr("قائمة المنتجات", "Products") : tr("تصنيفات القائمة", "Menu categories")}</h1><p className="mt-2 text-sm text-[#64716b]">{tr("ابحث عن السجل واضغط تعديل. الحقول ذات النجمة مطلوبة.", "Find a record and choose Edit. Fields marked * are required.")}</p></div>{!editing && <button className={`${button} bg-[#0e5a4f] text-white`} onClick={() => open()}><Plus size={18} />{tr("إضافة جديد", "Add new")}</button>}</div>
    {message && <p role="status" className="rounded-xl border bg-white p-4 text-sm">{message}</p>}
    {editing ? <form onSubmit={save} className="space-y-5 rounded-xl border bg-white p-5">
      <div className="flex items-center justify-between"><h2 className="text-lg font-semibold">{editing === "new" ? tr("إضافة جديد", "Add new") : tr("تعديل البيانات", "Edit details")}</h2><button type="button" disabled={saving} className={button} onClick={() => setEditing(null)}><ArrowLeft size={16} />{tr("إلغاء والعودة", "Cancel & return")}</button></div>
      ...
      {isProduct && <details className="rounded-xl border bg-white p-4"><summary className="cursor-pointer font-medium">{tr("إعداد أماكن التحضير", "Set up preparation areas")}</summary><form className="mt-4 grid gap-3 sm:grid-cols-3" onSubmit={async e => { e.preventDefault(); if (saving) return; setSaving(true); try { const r = await auth("/preparation-stations", { method: "POST", body: JSON.stringify(stationForm) }); if (!r.ok) throw new Error(); setStationForm({ code: "", nameAr: "", nameEn: "" }); await load(); } catch { setMessage(failure); } finally { setSaving(false); } }}>{(["code", "nameAr", "nameEn"] as const).map(k => <label key={k} className="text-sm">{k === "code" ? tr("رمز المكان *", "Area code *") : k === "nameAr" ? tr("الاسم بالعربية *", "Arabic name *") : tr("الاسم بالإنجليزية *", "English name *")}<input required className={input} maxLength={k === "code" ? 50 : 160} value={stationForm[k]} onChange={e => setStationForm({ ...stationForm, [k]: e.target.value })} /></label>)}<button disabled={saving} className={button}>{tr("إضافة مكان تحضير", "Add preparation area")}</button></form></details>}
    </>}
    {groupsFor && <section className="rounded-xl border bg-white p-5" aria-label={tr("إضافات المنتج", "Product modifiers")}><h2 className="font-semibold">{name(groupsFor)} — {tr("الإضافات", "Modifiers")}</h2>{groups.map(g => <label key={g.id} className="flex min-h-11 items-center gap-2"><input type="checkbox" checked={groupIds.includes(g.id)} onChange={e => setGroupIds(e.target.checked ? [...groupIds, g.id] : groupIds.filter(id => id !== g.id))} />{name(g)}</label>)}<div className="flex gap-3"><button className={button} disabled={saving} onClick={() => void saveGroups()}>{tr("حفظ الإضافات", "Save modifiers")}</button><button className={button} onClick={() => setGroupsFor(null)}>{tr("إغلاق", "Close")}</button></div></section>}
  </div>;
}
