...
export function AdvancedInventorySection({ language, permissions }: { language: Language; permissions: string[] | null }) {
  const t = copy[language];
  const can = (code: string) => permissions !== null && permissions.includes(code);
  const canCounts = can("inventory.counts.manage");
  const canTransfers = can("inventory.transfers.manage");
  const canWaste = can("inventory.waste.manage");
